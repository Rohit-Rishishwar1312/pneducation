              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  @foreach($data as $d)
                    <tr class="bg-light">
                      <td>{{$d->w_title}}</td>
                      <td><img src="{{ url('/uploade/'.$d->w_image) }}" style="height: 100px; width: 120px; border-radius: 100%;"></td>
                      <td>
                        <a href="{{url('addworkshop/edit/'.$d->id)}}">Edit</a>
                        <a href="{{url('addworkshop/delete/'.$d->id)}}">Delete</a>
                      </td>
                    </tr>
                  @endforeach
...
<h1>Add Workshop</h1>
<form method="post" action="{{url('addworkshop/insert')}}" enctype="multipart/form-data">
  @csrf
  Workshop Title:
